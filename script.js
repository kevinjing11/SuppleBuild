...
document.addEventListener("DOMContentLoaded", function () {
  ...
  async function submitImprovementsHandler() {
    ...
    if (!aiRequestCompleted) { //Await completion of AI Request, show spinner while doing so
      showSpinner();
    ...
    try {
      ...
      callBackendWithUpdatedValues(city, finalInputDict);
    } catch (error) {
  ...
  submitImprovementsBtn.onclick = async function () {
    ...
    await callBackendWithUpdatedValues(city, finalInputDict);
     ...
     if (aiResponseData && Array.isArray(aiResponseData.comments)) {
      console.log("callign displayAIComments with the following: ", aiResponseData.comments);
      displayAIComments(aiResponseData.comments);
    } else {
      console.error('No comments to display or comments are not in array format', aiResponseData ? aiResponseData.comments : 'Data not available');
    }
  ...
  function constructUserProfile() {
    return {
      age: document.getElementById("aiUserAge").value,
      gender: document.getElementById("aiUserGender").value,
      workRole: document.getElementById("aiUserRole").value,
      career: document.getElementById("aiUserCareer").value,
    };
  }
  ...
  document.getElementById("fetchButton").addEventListener("click", async function () {
    ...
    this.disabled = false; // Re-enable the button after completion
  })
});
